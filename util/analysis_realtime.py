...
# Main function for analysis
    ...
    def detect_face(self, frame):
        ...
        for face in faces:
            ...
            gaze_ratio_lr_right, gaze_ratio_ud_right = self.get_gaze_ratio(frame,
            ...
            cv2.putText(frame, emotions[self.emotion],
                       (50, 150), font, 2, (0, 0, 255), 3)
            #cv2.putText(frame, cis,
             #           (50, 250), font, 2, (0, 0, 255), 3)
            self.x = (gaze_ratio_lr + gaze_ratio_lr_right)/2
            self.y = (gaze_ratio_ud + gaze_ratio_ud_right)/2
            self.size = (left_eye_ratio + right_eye_ratio)/2
        return ci
...
# Downleft	2	1.5	0
        ...
        if self.size < 0.20:
            gaze_weights = 0
        elif self.size > 0.20 and self.size < 0.25:
            gaze_weights = 1.5
        else:
            if self.x < 2 and self.x > 0.6:
                gaze_weights = 5
            else:
                gaze_weights = 2
